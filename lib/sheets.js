...
async function deleteLastExpense(userId, startDate, endDate) {
  if (useWebApp()) {
    const response = await callWebApp("deleteLastExpense", { userId, startDate, endDate });
    return response.expense || null;
  }

  const rows = await getRows(EXPENSES_SHEET, true);
  const latest = rows
  ...
  await deleteRow(EXPENSES_SHEET, latest.rowNumber);
  return normalizeExpenseRow(latest);
}

async function updateLastExpense(userId, startDate, endDate, changes) {
  if (useWebApp()) {
    const response = await callWebApp("updateLastExpense", { userId, startDate, endDate, changes });
  ...
  const before = normalizeExpenseRow(latest);
  const after = { ...before, ...changes };
  await updateRow(EXPENSES_SHEET, latest.rowNumber, [
    after.userId,
    after.date,
    after.itemName,
    after.amount,
