...
- 確認コマンド：`残り` / `今日` / `一覧`
- 追加コマンド：`今週` / `多い順` / `取消`
- 日付指定：`昨日 カフェ 500` / `7/28 電車 220`
- 複数登録：`固定費 家賃70000 通信費3000`
- 直前修正：`さっきの600円じゃなくて650円`
- 簡易カテゴリ自動付与
- 残り予算が少ない場合の警告
...
無料運用したい場合は `OPENAI_ENABLED=false` のままにしてください。この状態ではOpenAI APIを呼ばず、ローカル解析だけで動きます。`OPENAI_API_KEY` も空欄で起動できます。
...
Botコードを更新して `取消` や `直前修正` を使う場合は、Apps Script側もこの `Code.gs` に貼り替えて、`新しいデプロイ` または `デプロイを管理` から新バージョンを公開してください。
